# logic/systems/biomass_system.py
import random
...
class BiomassSystem(IGameSystem):
    def on_init(self):
        # Храним данные о еде в самой системе (общий доступ для мутаций)
        self.game.food_items = {}  # {(cx, cy): [{"x":, "y":}, ...]}
        self.food_color = (0, 0.8, 1, 0.8)  # Голубой неон
        self.eat_dist = 35  # Дистанция обычного поедания (без мутаций)

    def on_chunk_generated(self, cx, cy, chunk_data):
        """Наполнение нового чанка биомассой"""
        ts = self.game.tile_size
        cs = self.game.chunk_size
        chunk_px = cs * ts

        food_list = []
        # Получаем графическую группу чанка из контейнера, созданного WorldSystem
        ...
        for _ in range(random.randint(3, 6)):
            ...
            # Проверяем наложение на препятствия через WorldSystem
    ...
    def on_update(self, dt):
        """Базовая логика поедания при контакте"""
        if not self.game.food_items:
            return
        ...
        # Проверяем только 9 ближайших чанков
            ...
            for cy in range(ccy - 1, ccy + 2):
                if (cx, cy) in self.game.food_items:
                    for food in self.game.food_items[(cx, cy)][:]:
                        ...
                        # Обычное поедание при соприкосновении (35 пикселей)
                        if dist_sq < self.eat_dist ** 2:
                            self.game.food_items[(cx, cy)].remove(food)
                            self.game.biomass_points += 1
                            # Мини-эффект вспышки головы (через damage_timer)
                            self.game.damage_timer = -0.1
                            self._refresh_food_graphics(cx, cy)
                            return # За один кадр съедаем одну частицу для стабильности

    def on_render(self, canvas, t, zoom):
        """Отрисовка пакетов еды через InstructionGroup чанков"""
        chunk_px = self.game.chunk_size * self.game.tile_size
        ccx = int(self.game.world_x // chunk_px)
        ccy = int(self.game.world_y // chunk_px)
    ...
    def _refresh_food_graphics(self, cx, cy):
        """Пересборка статичного пакета графики чанка при изменении состава еды"""
        if (cx, cy) in self.game.obstacles:
